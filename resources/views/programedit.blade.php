  ...
  <body style='font-family: Arial; background-color: #cde8f9;'>
    ...
    <div class='container text-center'>
      ...
      <div class='d-flex flex-row'>
        <div class='px-5 title'>Editeaza Echipaj</div>
      </div>
      
      <h4 class='text'>Toate campurile cu steluta (*) sunt obligatorii!</h4>
        <form class='text-left' id="edit-form" action="{{route('echipajeedit',['idechipaj' => $echipaj->idEchipaj])}}" method='post' enctype="multipart/form-data">
        @csrf
            <div class="group">
            <div class="form-group">
                <label class='title'>Pilot</label>
                   <select class="pilot" name="pilot">
                    <option ang="{{$echipaj->pilot->idAngajat}}" selected value="{{$echipaj->pilot->idAngajat}}" calificari="{{$echipaj->pilot->calificari}}">
                        {{$echipaj->pilot->nume.' '.$echipaj->pilot->prenume}}
                    </option>
                    @foreach($angajati as $ang)
                        @if($ang->tip_angajat == "pilot" && $ang->idAngajat != $echipaj->pilot->idAngajat)
                            <option ang="{{$ang->idAngajat}}" value="{{$ang->idAngajat}}" class="{{ $ang->idAngajat ==$echipaj->copilot->idAngajat?"hidden":"" }}" calificari="{{$ang->calificari}}">
            ...
            <div class="form-group">
               ...
               <select class="copilot" name="copilot">  
                    <option ang="{{$echipaj->copilot->idAngajat}}" selected  value="{{$echipaj->copilot->idAngajat}}" calificari="{{$echipaj->pilot->calificari}}">
                         {{$echipaj->copilot->nume.' '.$echipaj->copilot->prenume}}
                    </option>
                    @foreach($angajati as $ang)
                        @if($ang->tip_angajat == "pilot"  && $ang->idAngajat != $echipaj->copilot->idAngajat)
                            <option ang="{{$ang->idAngajat}}"  value="{{$ang->idAngajat}}" class="{{ $ang->idAngajat ==$echipaj->pilot->idAngajat?"hidden":"" }}"  calificari="{{$ang->calificari}}">
                                {{$ang->nume.' '.$ang->prenume}}
                            </option>
                         @endif
                    @endforeach
